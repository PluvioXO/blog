...
export default function Photos() {
      ...
      changeOverlay(
        ...
        margin: "auto"}} initial="hidden" animate="visible" variants={variants}>
          ...
          <div style={{width:'min-content', height:'680px', display: 'flex'}}>
            <div>
              <motion.img src={PhotoData[index].item} style={{borderRadius:'5px', height:'100%', width:'auto', userSelect: 'none'}} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.25, duration:1}}/> 
            </div>
            <motion.svg
              width='20px'
              height="680px"
            ...
            <div style={{margin:'20px', userSelect: 'none', textAlign:'right'}}>
              <div style={{backgroundColor:'#ededed', padding:'10px', borderRadius:'5px', marginBottom:'20px'}}>
                <h2 style={{fontWeight: "bold", fontSize: "1.5rem", color:'#046591', marginBottom:0}}>Date</h2>
                <span style={{marginBottom:0, whiteSpace:"nowrap", overflow:"hidden"}}>{PhotoData[index].dateFullForm}</span>
              </div>
              <div style={{backgroundColor:'#ededed', padding:'10px', marginBottom:'20px', borderRadius:'5px'}}>
                <h2 style={{fontWeight: "bold", fontSize: "1.5rem", color:'#046591'}}>Location</h2>
                <span style={{marginBottom:0, whiteSpace:"nowrap", overflow:"hidden"}}>{PhotoData[index].location}, {PhotoData[index].planet}</span>
                <span style={{marginBottom:0, whiteSpace:"nowrap", overflow:"hidden"}}>{PhotoData[index].coords}</span>
              </div>
              <div style={{backgroundColor:'#ededed', padding:'10px', marginBottom:'20px', borderRadius:'5px'}}>
                <h2 style={{fontWeight: "bold", fontSize: "1.5rem", color:'#046591'}}>Processing</h2>
                <span style={{marginBottom:0, whiteSpace:"nowrap", overflow:"hidden"}}>{PhotoData[index].processing}</span>
              </div>
            </div>
  ...
  {button}
  <center>
    <h6 id="contentsItem" style={{color:'#6B7280', fontWeight:'bolder'}}>13 Pictures | 0 Videos</h6>
  </center>
  </div>
  );
}
