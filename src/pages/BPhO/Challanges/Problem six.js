...
import { earthData, marsData, mercuryData, venusData, jupiterData, saturnData, uranusData, neptuneData, plutoData } from './Contents/Problem 3'
...
//Start of actual map
    ...
    const [currentGraph, updateGraph] = useState(<Scatter data={data1} options={{animation:false, plugins: {tooltip: false, legend: false}, maintainAspectRatio:true, aspectRatio:1 , scales:{y: {min:y1, max:y2, title: {display: true, text: 'y [AU]'}}, x: {min:x1, max:x2, title: {display: true, text: 'x [AU]'}}} }}/>)
    ...
    />
                ...
                <Col span={5}>
                    <div style={{padding:'10px', margin: '0', position: 'absolute', top: '50%', transform: 'translateY(-50%)', width: '100%'}}>
                        <Button type='primary' disabled={animation} style={{marginBottom: '10px', width:'100%'}} onClick={drawLine}>Start Animation</Button>
