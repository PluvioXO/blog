...
export default function ProblemOne () {
    ...
    const tex6= "$(R^2)$"
    const tex7= "$(r)$"
    const tex8= "$$R^2 = \\left[\\frac{n(\\sum xy)-(\\sum x)(\\sum y)}{\\sqrt{n(\\sum x^2-(\\sum x)^2)\\times n(\\sum y^2-(\\sum y)^2)}}\\right]^2$$"
    const tex9= "$$\\left[\\frac{64592.4076}{64592.4088}\\right]^2 = 0.999... \\thickapprox 1$$"
    const tex10= "$$R^2 = 1$$"
    const tex11= "$$\\sqrt{R^2} = r$$"
    const tex12= "$$\\sqrt{1} = 1$$"
    const tex13= "$$r = 1$$" 
    const tex14 = "$a$ [AU]"
    const tex15 = "$P$ [Years]"
    ...
    <div style={{padding:'10px', margin:'10px', backgroundColor:'#f5f5f5', borderRadius:'10px', bottom:0}}>
        <Row>
            <Col span={12}>
                ...
                <Latex>{tex9}</Latex>
                <Latex>{tex10}</Latex>
                By understanding that the Coefficient of Determination <Latex>{tex6}</Latex> is the square of the Pearson correlation coefficient <Latex>{tex7}</Latex> we can square route our answer to find <Latex>$r$</Latex>.
